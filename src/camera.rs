...
pub(crate) mod rts {
    ...
    #[cfg(not(target_arch = "wasm32"))]
    fn pan_orbit_camera(time: Res<Time>, windows: Res<Windows>, mut state: ResMut<InputState>, mouse_motion_events: Res<Events<MouseMotion>>, mouse_button_input: Res<Input<MouseButton>>, mouse_scroll_events: Res<Events<MouseWheel>>, query: Query<(&mut CameraState, &mut Transform)>) {
        ...
        pan_orbit_camera_inner(&translation, &rotation_move, time, windows, state, mouse_scroll_events, query);
    }
    ...
    #[cfg(target_arch = "wasm32")]
    fn pan_orbit_camera(time: Res<Time>, windows: Res<Windows>, state: ResMut<InputState>, keyboard_input: Res<Input<KeyCode>>, mouse_scroll_events: Res<Events<MouseWheel>>, query: Query<(&mut CameraState, &mut Transform)>) {
        let mut translation = Vec2::zero();
        let mut rotation_move = Vec2::default();
    ...
    fn pan_orbit_camera_inner(translation: &Vec2, rotation_move: &Vec2, time: Res<Time>, windows: Res<Windows>, mut state: ResMut<InputState>, mouse_scroll_events: Res<Events<MouseWheel>>, mut query: Query<(&mut CameraState, &mut Transform)>) {
        let mut scroll = 0.0;
        ...
        for (mut camera, mut transform) in query.iter_mut() {
            if rotation_move.length_squared() > 0.0 {
                ...
                let delta_x = (rotation_move.x * camera.rotation_speed) / window_w * std::f32::consts::PI * 2.0;
                let delta_y = (rotation_move.y * camera.rotation_speed) / window_h * std::f32::consts::PI;
                ...
                transform.translation = delta_rotation * (transform.translation - camera.focus) + camera.focus;

                let look = Mat4::face_toward(transform.translation, camera.focus, Vec3::new(0.0, 0.0, 1.0));
                ...
                let mut translation = (strafe_vector * -translation.x + forward_vector * -translation.y) * time.delta_seconds;
...
pub(crate) mod fps {
    ...
    #[cfg(not(target_arch = "wasm32"))]
    fn mouse_movement_system(time: Res<Time>, mouse_motion_events: Res<Events<MouseMotion>>, mut query: Query<(&mut CameraState, &mut Transform)>) {
    ...
    /// the "forward vector" 90 degrees to compute the "strafe vector".
    fn strafe_vector(forward_vector: Vec3) -> Vec3 {
        Quat::from_rotation_y(90.0f32.to_radians()).mul_vec3(forward_vector)
    }
}
